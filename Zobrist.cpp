...
uint64_t Zobrist::zobristTable[781] ={
...
// Table pour la prise en passant (8 colonnes (a => 0, b => 1, ..., h => 7))
uint64_t Zobrist::zobristEnPassant[8] =
{
...
uint64_t Zobrist::computeZobristHash(const std::vector<int>& in_boardVectorIndexInZobristTable, bool in_bIsWhiteTurn, const std::vector<int>& in_vectIndexCastlingRights, int in_iIndexZobristEnPassant, bool in_bEnemyPawnNextToEnPassant) {
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        if (iIndexZobristPiece != -1) {
            hashedBoard ^= zobristTable[(64 * iIndexZobristPiece) + i];
        }
    }
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        {
            hashedBoard ^= zobristTable[768 + iIndiceCastling];
        }
    }
    ...
    if (in_bIsWhiteTurn) {
        hashedBoard ^= zobristTable[780]; // Dernière case
    }
